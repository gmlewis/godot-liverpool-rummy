...
func _ready() -> void:
	...
	Global.meld_area_state_changed.connect(_on_meld_area_state_changed)

func _exit_tree() -> void:
	Global.meld_area_state_changed.disconnect(_on_meld_area_state_changed)

func _on_meld_area_state_changed(is_complete: bool, area_index: int) -> void:
	Global.dbg("GML: ShaderSwitcher(%d) meld_area_state_changed: is_complete=%s, area_index=%d" % [meld_area_index, str(is_complete), area_index])
